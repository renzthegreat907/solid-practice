...
def test_place_on_out_of_bounds():
    ...
    assert model.place_on(Cell(0, 0)) == Feedback.OUT_OF_BOUNDS
    assert model.place_on(Cell(1, 0)) == Feedback.OUT_OF_BOUNDS
    assert model.place_on(Cell(0, 1)) == Feedback.OUT_OF_BOUNDS
    assert model.place_on(Cell(-1, 0)) == Feedback.OUT_OF_BOUNDS
    assert model.place_on(Cell(0, -1)) == Feedback.OUT_OF_BOUNDS
    assert model.place_on(Cell(-1, -1)) == Feedback.OUT_OF_BOUNDS
    assert model.place_on(Cell(0, 4)) == Feedback.OUT_OF_BOUNDS
    assert model.place_on(Cell(4, 0)) == Feedback.OUT_OF_BOUNDS
    assert model.place_on(Cell(1, 4)) == Feedback.OUT_OF_BOUNDS
    assert model.place_on(Cell(4, 1)) == Feedback.OUT_OF_BOUNDS


def test_place_on_all_valid():
    model = GridGameModel(3, [Symbol.CROSS, Symbol.NOUGHT])

    assert model.place_on(Cell(1, 1)) == Feedback.VALID
    assert model.place_on(Cell(2, 2)) == Feedback.VALID
    assert model.place_on(Cell(3, 3)) == Feedback.VALID
...
def test_place_on_game_over():
    ...
    model.place_on(Cell(1, 1))
    model.place_on(Cell(2, 2))
    model.place_on(Cell(1, 2))

    assert model.place_on(Cell(2, 1)) == Feedback.GAME_OVER
    assert model.place_on(Cell(2, 2)) == Feedback.GAME_OVER
...
def test_get_next_player_initial():
    ...
    assert model.get_next_player() == 2
...
def test_get_next_player_multiple_2_all_valid():
    ...
    assert model.get_next_player() == 2
    model.place_on(Cell(1, 1))
    assert model.get_next_player() == 1
    model.place_on(Cell(2, 2))
    assert model.get_next_player() == 2
    model.place_on(Cell(2, 1))
    assert model.get_next_player() == 1
...
def test_get_next_player_multiple_2_with_errors():
    ...
    model.place_on(Cell(-1, -1))
    model.place_on(Cell(-1, -1))
    model.place_on(Cell(0, 0))
    model.place_on(Cell(4, 1))
    assert model.get_next_player() == 2
    model.place_on(Cell(1, 3))
    assert model.get_next_player() == 1
    model.place_on(Cell(1, 3))
    model.place_on(Cell(1, 3))
    model.place_on(Cell(1, 3))
...
def test_get_next_player_multiple_3_all_valid():
    ...
    assert model.get_next_player() == 2
    model.place_on(Cell(1, 1))
    assert model.get_next_player() == 3
    model.place_on(Cell(2, 2))
    assert model.get_next_player() == 1
    model.place_on(Cell(2, 1))
    assert model.get_next_player() == 2
    model.place_on(Cell(3, 3))
    assert model.get_next_player() == 3
    model.place_on(Cell(3, 2))
    assert model.get_next_player() == 1
...
def test_has_unoccupied_cell_3():
    ...
    model.place_on(Cell(3, 2))
    assert model.has_unoccupied_cell()
    model.place_on(Cell(3, 1))
    assert model.has_unoccupied_cell()
    model.place_on(Cell(1, 3))
    assert model.has_unoccupied_cell()
    model.place_on(Cell(2, 3))
...
def test_get_winner_3_backslash():
    ...
    model.place_on(Cell(1, 1))  # X
    assert model.get_winner() is None
    model.place_on(Cell(1, 2))  # O
    assert model.get_winner() is None
    model.place_on(Cell(2, 2))  # X
    assert model.get_winner() is None
    model.place_on(Cell(2, 1))  # O
    assert model.get_winner() is None
    model.place_on(Cell(3, 3))  # X
    assert model.get_winner() == 1
...
def test_get_winner_3_forward_slash():
    ...
    model.place_on(Cell(1, 1))  # X
    assert model.get_winner() is None
    model.place_on(Cell(1, 3))  # O
    assert model.get_winner() is None
    model.place_on(Cell(2, 1))  # X
    assert model.get_winner() is None
    model.place_on(Cell(2, 2))  # O
    assert model.get_winner() is None
    model.place_on(Cell(3, 2))  # X
    assert model.get_winner() is None
    model.place_on(Cell(3, 1))  # O
    assert model.get_winner() == 2
...
def test_get_winner_3_row_1():
    ...
    model.place_on(Cell(2, 1))  # O
    assert model.get_winner() is None
    model.place_on(Cell(1, 2))  # X
    assert model.get_winner() is None
    model.place_on(Cell(2, 2))  # O
    assert model.get_winner() is None
    model.place_on(Cell(1, 3))  # X
    assert model.get_winner() == 1
...
def test_get_winner_3_row_2():
    ...
    model.place_on(Cell(2, 1))  # X
    assert model.get_winner() is None
    model.place_on(Cell(1, 1))  # O
    assert model.get_winner() is None
    model.place_on(Cell(2, 2))  # X
    assert model.get_winner() is None
    model.place_on(Cell(1, 2))  # O
    assert model.get_winner() is None
    model.place_on(Cell(2, 3))  # X
    assert model.get_winner() == 1
...
def test_get_winner_3_col_3():
    ...
    model.place_on(Cell(3, 1))  # X
    assert model.get_winner() is None
    model.place_on(Cell(2, 3))  # O
    assert model.get_winner() is None
    model.place_on(Cell(3, 2))  # X
    assert model.get_winner() is None
    model.place_on(Cell(3, 3))  # O
    assert model.get_winner() == 2
...
def test_get_grid_size():
    assert GridGameModel(
        2, [Symbol.NOUGHT, Symbol.CROSS]).get_grid_size() == 2
    assert GridGameModel(
        3, [Symbol.NOUGHT, Symbol.CROSS]).get_grid_size() == 3
    assert GridGameModel(
        4, [Symbol.NOUGHT, Symbol.CROSS]).get_grid_size() == 4
    assert GridGameModel(
        5, [Symbol.NOUGHT, Symbol.CROSS]).get_grid_size() == 5
    assert GridGameModel(
        10, [Symbol.NOUGHT, Symbol.CROSS]).get_grid_size() == 10
